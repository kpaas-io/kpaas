...
func newCert(d *deployEtcdOperation) (*tls.Certificate, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("failed to parse certificate for etcd client:%v, error: %v", d.m.Name, err)
	}
...
func getClientV3TLS(d *deployEtcdOperation) (*tls.Config, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("failed to get cert pool for etcd client:%v, error:%v", d.m.Name, err)
	}
