...
func TestNodeInitExecute(t *testing.T) {
	executor := new(nodeInitExecutor)

	normalAction, err := NewNodeInitAction(&NodeInitActionConfig{
	...
	errorAction, err := NewNodeInitAction(&NodeInitActionConfig{
		NodeInitConfig: &pb.NodeDeployConfig{
			Node: &pb.Node{
				Name: "error",
				Ip:   "10.10.10.10",
