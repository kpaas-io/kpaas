...
func TestInitMaster(t *testing.T) {
	executor := new(initMasterExecutor)

	normalAction, err := NewInitMasterAction(&InitMasterActionConfig{
		Node: &pb.Node{
			Name: "normal",
			Ip:   "10.10.10.10",
		},
	...
	errorAction, err := NewInitMasterAction(&InitMasterActionConfig{
		Node: &pb.Node{
			Name: "error",
			Ip:   "10.10.10.10",
		},
